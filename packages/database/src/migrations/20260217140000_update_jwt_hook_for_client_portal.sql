-- Update JWT Custom Claims Hook for Client Portal
-- Extends the existing hook to support client_portal_users alongside user_profiles.
-- Priority: user_profiles first (staff), then client_portal_users, hard-fail if neither.
...
BEGIN
  ...
  IF user_agency_id IS NOT NULL AND user_status = 'active' THEN
    -- Client portal user found and active - set client claims
    claims := event->'claims';
    claims := jsonb_set(claims, '{agency_id}', to_jsonb(user_agency_id));
