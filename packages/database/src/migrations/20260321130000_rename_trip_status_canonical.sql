...
-- Fully idempotent — safe on fresh, partially-migrated, and fully-migrated DBs.
-- PREREQUISITE: deploy workflow pre-migration step adds new enum values first.

-- Drop triggers unconditionally (idempotent)
...
-- Update data (no-op if already migrated)
...
-- Swap enum (skip if already canonical)
DO $$
BEGIN
...
-- Recreate function (always — idempotent via CREATE OR REPLACE)
...
-- Recreate triggers via DO block with EXECUTE to handle the enum cast properly
DO $$
BEGIN
  EXECUTE 'CREATE TRIGGER validate_trip_status_transition_trigger BEFORE INSERT OR UPDATE ON trips FOR EACH ROW EXECUTE FUNCTION validate_trip_status_transition()';
  EXECUTE 'CREATE TRIGGER set_trip_reference_update BEFORE UPDATE ON trips FOR EACH ROW WHEN (NEW.status = ''planning''::trip_status AND OLD.trip_type IS DISTINCT FROM NEW.trip_type) EXECUTE FUNCTION generate_trip_reference()';
END $$;
