...
import { ActivityMediaService } from '../trips/activity-media.service'
...
export class OcrImportService {
  ...
  async preview(
    ...
    try {
      // First extraction pass (without supplier-specific hints)
      let extraction = await this.ocrService.extractFromPdf(
        file.buffer,
        { documentType, tripId: dto.tripId, contactId: dto.contactId },
        abortController.signal,
      )
      clearTimeout(timeoutId)

      // Resolve supplier name: prefer detection-level, fall back to extraction-level
      const supplierName = extraction.supplierName
        || this.getExtractedSupplierName(extraction)

      // Look up runbook by supplier + document type
      ...
      if (supplierName) {
        ...
        if (runbook) {
          ...
          if (extraction.confidence > 0.6) {
            this.logger.log({
              message: 'Re-extracting with supplier hints',
              supplierName,
              runbookId: runbook.id,
            })
      ...
      // 8. Build response
      return await this.buildPreviewResponse(job.id, extraction, contactMatches)
    } catch (error) {
      clearTimeout(timeoutId)
  ...
  async getJobStatus(jobId: string, auth: AuthContext): Promise<OcrJobStatusResponse> {
    ...
    if (job.status === 'preview_ready' && job.extractionResult) {
      ...
      response.preview = await this.buildPreviewResponse(job.id, extraction, contactMatches)
    }
  ...
  private async confirmCruise(
    ...
    try {
      ...
      if (match) {
        this.logger.log({
          message: 'Matched catalog sailing for OCR cruise',
        ...
        this.logger.log({
          ...
          shipGalleryImages: enrichment.shipGalleryImages.length,
          deckPlanImages: enrichment.deckPlanImages.length,
          hasCruiseLineLogo: !!enrichment.cruiseLineLogo,
        ...
        await this.db.client
          ...
          .set({
            ...
            // Use catalog canonical dates for consistency
            departureDate: enrichment.canonicalSailDate,
            arrivalDate: enrichment.canonicalEndDate,
            // Persist OCR-extracted times and cabin deck
            departureTime: cruise.departureTime || undefined,
            arrivalTime: cruise.arrivalTime || undefined,
            cabinDeck: cruise.cabinDeck || undefined,
            // Port names from catalog
            departurePort: enrichment.departurePort || cruise.departurePort || null,
            arrivalPort: enrichment.arrivalPort || cruise.arrivalPort || null,
            portCallsJson: enrichment.portCallsJson,
            updatedAt: new Date(),
          })
          .where(eq(customCruiseDetails.activityId, activity.id))

        // Import all available catalog media into activity_media
        try {
          const mediaImages: Array<{ url: string; caption?: string; attribution?: { source: string; sourceUrl?: string } }> = []

          // Ship gallery images (hero first, then gallery)
          if (enrichment.shipGalleryImages.length > 0) {
            for (const img of enrichment.shipGalleryImages) {
              mediaImages.push({
                url: img.url,
                caption: img.caption || (img.isHero ? `${cruise.shipName || 'Ship'}` : undefined),
                attribution: { source: 'traveltek', sourceUrl: img.url },
              })
            }
          } else if (enrichment.shipImageUrl) {
            // Fallback: single ship image from cruiseShips.imageUrl
            mediaImages.push({
              url: enrichment.shipImageUrl,
              caption: cruise.shipName || 'Ship',
              attribution: { source: 'traveltek', sourceUrl: enrichment.shipImageUrl },
            })
          }

          // Cruise line logo
          if (enrichment.cruiseLineLogo) {
            mediaImages.push({
              url: enrichment.cruiseLineLogo,
              caption: `${cruise.cruiseLineName || 'Cruise Line'} Logo`,
              attribution: { source: 'traveltek', sourceUrl: enrichment.cruiseLineLogo },
            })
          }

          // Deck plan images
          for (const deck of enrichment.deckPlanImages) {
            mediaImages.push({
              url: deck.url,
              caption: deck.caption,
              attribution: { source: 'traveltek', sourceUrl: deck.url },
            })
          }

          if (mediaImages.length > 0) {
            const batchResult = await this.activityMediaService.importExternalImagesBatch(
              activity.id,
              mediaImages,
              'cruise',
            )
            this.logger.log({
              message: 'Imported catalog media for cruise',
              activityId: activity.id,
          ...
          this.logger.warn({
            message: 'Failed to import catalog media — non-blocking',
            activityId: activity.id,
            error: mediaError instanceof Error ? mediaError.message : String(mediaError),
          })
        }

        // Generate port schedule from catalog port calls
        if (enrichment.portCallsJson.length > 0) {
          try {
            await this.componentOrchestrationService.generateCruisePortSchedule(
              activity.id,
              {
                ...
                customCruiseDetails: {
                  ...
                  arrivalDate: enrichment.canonicalEndDate,
                  portCallsJson: enrichment.portCallsJson,
                  departurePort: enrichment.departurePort || cruise.departurePort,
                  arrivalPort: enrichment.arrivalPort || cruise.arrivalPort,
                },
                skipDelete: true,
            ...
            this.logger.warn({
              message: 'Failed to generate port schedule for OCR cruise — non-blocking',
              activityId: activity.id,
        ...
        this.logger.log({
          message: 'No catalog match found for OCR cruise — using OCR data only',
          cruiseLine: cruise.cruiseLineName,
          ship: cruise.shipName,
          departureDate: cruise.departureDate,
          voyageCode: cruise.voyageCode,
        })

        // Still persist OCR-extracted times and cabin deck even without catalog match
        if (cruise.departureTime || cruise.arrivalTime || cruise.cabinDeck) {
          await this.db.client
            .update(customCruiseDetails)
            .set({
              departureTime: cruise.departureTime || undefined,
      ...
      this.logger.warn({
        message: 'Failed to enrich cruise from catalog — non-blocking',
  ...
  private async buildPreviewResponse(
    ...
    // Attempt catalog match for cruise documents
    ...
    if (cruiseData && extraction.documentType === 'cruise_confirmation') {
      try {
        ...
        if (match) {
          // Fetch ship + cruise line names for display
          let catalogShipName: string | null = null
          let catalogLineName: string | null = null
          try {
            const [shipRow] = await this.db.client
              .select({ name: cruiseShips.name })
              .from(cruiseShips)
              .where(eq(cruiseShips.id, match.shipId))
              .limit(1)
            catalogShipName = shipRow?.name ?? null

            const [lineRow] = await this.db.client
              .select({ name: cruiseLines.name })
              .from(cruiseLines)
              .where(eq(cruiseLines.id, match.cruiseLineId))
              .limit(1)
            catalogLineName = lineRow?.name ?? null
          } catch {
            // Non-critical — names are just for display
          }
          ...
          cruiseData = {
            ...
            catalogMatch: {
              matched: true,
              providerIdentifier: match.providerIdentifier,
              strategy: match.strategy,
              sailingId: match.sailingId,
              shipName: catalogShipName,
              cruiseLineName: catalogLineName,
            },
          }
          this.logger.log(`Preview catalog match: ${match.strategy}, providerIdentifier=${match.providerIdentifier}`)
        } else {
          cruiseData = {
            ...cruiseData,
            catalogMatch: { matched: false },
          }
        }
      } catch (err) {
        this.logger.warn(`Catalog match during preview failed: ${err instanceof Error ? err.message : err}`)
        cruiseData = {
          ...cruiseData,
          catalogMatch: { matched: false },
        }
  ...
  /**
   * Look up a runbook by supplier name + document type.
   * Deterministic ranking: exact normalized match first, ILIKE substring fallback second.
   */
  async findRunbookBySupplier(
    supplierName: string,
    documentType: string,
    ...
    try {
      ...
      // 2. Fallback: ILIKE substring match (supplier name contains the search term or vice versa)
  ...
  /**
   * Extract supplier name from nested extraction fields (fallback when detection didn't return one).
   */
  private getExtractedSupplierName(extraction: OcrExtractionResult): string | null {
    const info = this.getSupplierInfo(extraction)
    return info?.supplierName || null
  }
  ...
  private async incrementRunbookSuccess(jobId: string): Promise<void> {
    try {
      const [job] = await this.db.client
        .select({
          runbookId: ocrImportJobs.runbookId,
          hintsApplied: ocrImportJobs.hintsApplied,
        })
        .from(ocrImportJobs)
        .where(eq(ocrImportJobs.id, jobId))
        .limit(1)

      // Only increment when hints were actually applied during extraction
      if (job?.runbookId && job.hintsApplied) {
        await this.db.client
